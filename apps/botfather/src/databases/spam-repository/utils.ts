...
const validateMessageInput = (input: CreateMessageInput, keyName: string) => {
  if (!(keyName in input)) {
    throw new Error(
      `Invalid message input: ${JSON.stringify(
        input
      )}, must have a ${keyName} property`
    );
  }
  if (input[keyName].length === 0) {
    throw new Error(
      `Invalid message input: ${JSON.stringify(
        input
      )}, ${keyName} property must not be empty`
    );
  }
};

export const validateMessageInputByType = (input: CreateMessageInput) => {
  const { type } = input;
  if (!MessageTypeValues.includes(type as EMessageType)) {
    throw new Error(
      `Invalid message type: ${type}, must be one of ${MessageTypeValues.join(
        ", "
      )}`
    );
  }
